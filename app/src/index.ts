      ...
      window.addEventListener('scroll', function(e) {
        ...
        if (that.time_to_update_toc) {
          this.window.requestAnimationFrame(function() {
            ...
            let header_num =
                that.GetCurrentActiveTOCItem(last_known_scroll_pos);
            that.HighlightActiveItem(header_num);
            console.log(last_known_scroll_pos, header_num);
            console.log(that.toc_headers_);
          });
...
// Main entry point.
function Main() {
  TOCModule.BuildTableOfContents();
}
