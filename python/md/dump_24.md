...
`3[arr]` 은 무언가 조금 이상한 표현 입니다. 사실 이렇게 사용한다면 가독성도 떨어지고 한 번에 이해도 되지 않기에 99.9999% 의 프로그래머들은 `arr[3]` 으로 사용할 것입니다. 하지만, 앞에서도 `[]` 는 연산자로 `3[arr]` 을 `*(3+arr)` 로 바꿔주기 때문에 `arr[3]` 과 동일한 결과를 출력할 수 있게 되지요.
