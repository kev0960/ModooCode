...
`C` 언어에서 `fseek` 과 같은 함수로, 파일 위치 지정자를 사용자의 입맛에 맞게 이리저리 움직일 수 있습니다. 두 번째 인자는, 파일 내 위치를 의미하고, 첫 번째 인자는 그 위치로 부터 얼마나 만큼 떨어져 있느냐를 의미합니다. 우리의 경우 위치 지정자를 파일의 끝에서 0 만큼 떨어진 것, 즉 파일의 끝으로 이동시켰습니다 C 언어에서 `fseek` 과 같은 함수로, 파일 위치 지정자를 사용자의 입맛에 맞게 이리저리 움직일 수 있습니다. 두 번째 인자는, 파일 내 위치를 의미하고, 첫 번째 인자는 그 위치로 부터 얼마나 만큼 떨어져 있느냐를 의미합니다. 우리의 경우 위치 지정자를 파일의 끝에서 0 만큼 떨어진 것, 즉 파일의 끝으로 이동시켰습니다.
...
또한 ExprCell 의 경우 간단한 수식에 대한 정보를 가지는 객체로, C`ell` 들 간의 연산을 사용할 수 있습니다. 에를 들어서 "A1+B2+C6-6" 와 같은 데이터가 들어 있는 ExprCell 에 `to_numeric` 함수를 호출하면`A1, B2, C6` 의 값을 더하고 (각 셀에 `to_numeric` 을 해서), 6 을 빼준 결과값이 나와야 합니다.
