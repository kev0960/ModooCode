...
반면에 `unsigned int` 는 양수만을 표현할 수 있는 대신에, 양수로 표현할 수 있는 범위가 두 배로 늘어납니다. 또한 마지막에 보면 `float, double, long double` 이 있는데 이들은 '실수형' 자료형으로 소수(`0.1, 1.4123` 등) 을 표현 할 수 있습니다. 뿐만 아니라 `double` 의 경우,
