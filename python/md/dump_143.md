...
스트림과 작업하기 위해서는 C++ 은 아래와 같은 **표준 입출력 라이브러리 (standard iostream library)** 를 지원하여, 아래와 같은 것들을 가지고 있다. (참고적으로 C 의 경우 C 표준 입출력 라이브러리 (`<stdio>`) 를 지원하며 C++ 에서 역시 사용 가능한데, 이 경우 `<cstdio>` 를 참조하면 된다)
