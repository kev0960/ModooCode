...
참고로 우리가 따로 생성자를 정의하지 않더라도 디폴트 생성자가 있었던 것 처럼, 소멸자도 디폴트 소멸자(Default Destructor) 가 있습니다. 물론, 아무런 역할은 하지 않습니다. 만일 소멸자가 필요 없는 클래스라면 굳이 소멸자를 따로 써줄 필요는 없습니다.
...
처럼 pc 의 값 자체는 변경할 수 없다는 이야기 입니다. (왜냐하면 const 레퍼런스로 인자를 받았기 때문이죠! - 아직도 이해가 안되시면 이전에 포인터에서 const 의 용법을 떠올려보시기 바랍니다. 정확히 하는 동작이 동일합니다.) 
