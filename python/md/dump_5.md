...
이제, 컴파일러를 어디서 구할 수 있는지 찾아봅시다. 유명한 컴파일러로는 무료로 배포되는 `gcc` 와 `clang` 를 들 수 있지만 사용법이 초보자들에게는 약간 복잡하므로, 마이크로소프트에서 무료로 배포하는 `Visual Studio 2017 Community Edition` 을 사용하도록 합니다. 여기에는 컴파일러 뿐만이 아니라, 사용자들이 프로그래밍 하기 편하게 여러가지 프로그램들이 딸려 옵니다. 땡큐죠!
