...
![](http://img1.daumcdn.net/thumb/R1920x0/?fname=http%3A%2F%2Fcfile23.uf.tistory.com%2Fimage%2F194C49214B01139B70E9DA)
...
과 같은 형태임을 알 수 있습니다. 결과적으로 배열의 이름은 죽어다 깨어나도 언제나 배열의 첫번째 원소를 가리키게 됩니다.

배열의 이름이 결코 다른 값들을 가리킬 수 없도록 C 에서 제한을 두는 이유는, 제 생각에 프로그래머가 실수로 배열의 이름이 가리키는 값을 잘못 바꿔서 배열을 '아무도 가리키지 않는 메모리 상의 미아'가 됨을 막으려고 한 것이 아닐까요?
...
`arr[0]` 의 값이 `arr[0][0]` 의 주소값과 같고, `arr[1]` 의 값이 `arr[1][0]` 의 주소값과 같습니다. 이것을 통해 알 수 있는 사실은 바로 `arr[0]` 은 `arr[0][0]` 을 가리키는 포인터 이고, `arr[1]` 은 `arr[1][0]` 을 가리키는 포인터라는 뜻이 되겠지요. 이 때, `arr[0][0]` 의 형이 `int` 이므로 `arr[0]` 은 `int*` 형이 되겠고, 마찬가지로 `arr[1]` 도 `int*` 형이 되겠지요.

한 가지 중요한 점은 `arr[0]` 의 값이 `arr[0][0]` 의 주소값과 같다고 해서, `arr[0]` 이라는 **포인터가 메모리에 존재하는 것은 아니라는 점**입니다. 
