...
[3 강](/7) 에서 우리는  `%f` 가 오직 실수형 데이터 만을 출력하기 위해 있는 것이라 하였습니다. 그런데, `a / b` 가 `3.3333` 이라고 해서 실수형 데이터가 되는 것이 아니라, `(정수형 변수) (연산) (정수형 변수)` 는 언제나 `(정수)` 이기 때문에 실수형 데이터를 출력하는 `%f` 를 이용하면 이와 같이 이상한 결과나 나오게 됩니다.
