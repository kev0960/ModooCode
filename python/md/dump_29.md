...
반면의 `not_null` 의 경우 문자 '0' 의 아스키값이 들어가는데, 문자 0 의 아스키값은 48 입니다. 따라서, `not_null` 의 정수값을 출력할 때 48 가 출력되었습니다. 위 예제를 통해 우리는 문자열의 마지막에는 종료 문자로 `'\0'` 이나 `NULL` 혹은 문자 0 이 아닌 0 이란 값 자체를 사용할 수 있음을 알았습니다.
