...
class Comment():
    ...
    def __str__(self):
        return "({0}-{1}) ({2}) at ({3}) : ({4}), repls : ({5})".format(self.uniq, self.article_url, self.author, self.timestamp,
                                                                        self.content, self.reply)

    def to_obj(self):
        return {'uniq': self.uniq, 'author': self.author, 'timestamp': self.timestamp,
                'content': self.content, 'article': self.article_url, 'reply': self.reply}


class InsertCommentToDb():
    def __init__(self):
        self.conn = pg2.connect(host="localhost",
    ...
    def insert_comment_to_db(self, article_url, comment_list):
        for c in comment_list:
            ...
            timestamp = c[2]
            content = c[3]
            comm = Comment(self.current_unique, author,
                           timestamp, content, article_url)
    ...
    def do_insert(self):
        sql = """INSERT INTO comment(comment_id, article_url, reply_ids, vote_up, vote_down, comment_date, modified_date,
              ...
              """
        for c in self.comment_list:
            self.cur.execute(sql, (c.uniq, c.article_url, c.reply, 0, 0, c.timestamp, c.timestamp, c.author,
                                   "", "", c.content, ""))
        self.conn.commit()
        ...
        if tag == 'li':
            ...
            if self.current_div[0] == 'content':
                ...
                self.comment_list.append(
                    [self.li_depth, self.comment_name, self.comment_date, self.comment_content])
...
if __name__ == '__main__':
    ...
    with tqdm(os.listdir('./blog')) as t:
        for filename in t:
            ...
            #   continue
            comment_parser = BlogCommentParser(
