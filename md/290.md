...
위와 같은 오류가 발생하게 되지요. 그 이유는 **C++ 컴파일러는 함수를 컴파일 시에, 자신의 앞에 정의되어 있는 함수들 밖에 보지 못하기 때문**입니다. 따라서 `void print(T arg, Types... args)` 이 함수를 컴파일 할 때, `void print(T arg)` 이 함수가 존재함을 모르는 셈이지요.
...
예를 들어서 모든 인자들을 다 더하는 함수를 만들어본다고 생각해봅시다.
