...
위 코드는 libc++ 라이브러리에서 가져온 코드로, `unique_ptr` 의 주소값을 출력해주는 `basic_ostream` 의 `operator<<` 연산자를 구현한 것입니다. 도대체 왜 C++ 개발자들은 저런 혐오스러운 코드를 작성하는 것일까요? \sidenote{여담이지만 대표적인 C++ 표준 라이브러리르 GCC 에서 사용하는 libstdc++ 와 Clang 에서 사용하는 libc++ 을 뽑을 수 있습니다.}
