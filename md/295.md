...
보시다시피 일반적인 함수와 몇 가지 차이점이 있습니다. 가장 중요한 점은 템플릿 메타 함수들은 실제론 **함수가 아니라는 점** 입니다. 만일 함수였다면 `()` 를 통해서 호출을 했겠지요. 하지만 is_void 는 그렇지 않습니다. `()` 대신에 `<>` 를 통해 함수 인자가 아닌 **템플릿 특수화 인자를 전달**하고 있습니다. 실제로 is_void 는 클래스로 구현되어 있습니다.

#### is_void

기존에 템플릿 메타프로그래밍 강좌를 숙지하신 분들은 알고 계시겠지만, TMP 에서 if 문은 템플릿 특수화를 통해서 구현된다고 하였습니다. is_void 의 경우도 마찬가지 입니다. 
...
만약에 static_assert 에 전달된 bool 타입의 constexpr 가 **참** 이라면, 컴파일러에 의해 해당 식은 무시되고, **거짓** 이라면 해당 문장에서 컴파일 오류를 발생시키게 됩니다.
...
type_traits 에 정의되어 있는 메타 함수들 중에서 한 가지 흥미로운 함수로 is_class 가 있습니다. 이 메타 함수는 인자로 전달된 타입이 **클래스 인지 아닌지 확인하는 메타 함수** 입니다.
...
이 부분이 T 가 클래스 라면 참이고 클래스가 아니라면 거짓이 된다는 뜻입니다. 
...
```cpp
    template <class T> char test(int T::*); // (1)
```
...
참고로 is_union 이 어떻게 구현되어 있는지 궁금해 하실 수 있는데, 안타깝게도 C++ 에선 클래스와 공용체는 구별할 수 있는 방법이 **없습니다**. 따라서 is_union 은 [컴파일러에 직접 의존한 방식으로 구현](https://libcxx.llvm.org/type_traits_design.html)되어 있기 때문에 자세한 내용은 생략하겠습니다. 
...
이 때 `B` 부분에 우리가 확인하고픈 조건을 전달한다면, 만일 `B` 가 참으로 연산된다면 enable_if::value 의 타입이 `T` 가 되고, `B` 가 거짓이라면 enable_if 에 value 가 존재하지 않게 됩니다. 예를 들어서, 어떤 함수의 인자 T 가 정수 타입일 때만 오버로딩을 하고 싶다고 해봅시다. 그렇다면 해당 작업을 하는 enable_if 는 아래와 같이 정의되겠네요.
...
실제 enable_if 를 사용한 함수는 아래와 같이 쓸 수 있습니다.
...
그렇다면 만약에 func2 말고도 여러 개의 함수를 확인하고 싶다면 어떨까요? 예를 들어서 C++ 컨테이
