...
void producer(queue<string>* downloaded_pages, mutex* m, int index) {
  for (int i = 0; i < 5; i++) {
    ...
    string content =
      "웹사이트 : " + to_string(i) + " from thread(" + to_string(index) + ")\n";
...
int main() {
  ...
  for (int i = 0; i < 3; i++) {
    consumers.push_back(
      thread(consumer, &downloaded_pages, &m, &num_processed));
  }
...
`this_thread::sleep_for` 함수는 인자로 전달된 시간 만큼 쓰레드를 `sleep` 시키는데, 이 때 해당 인자로 `chrono` 의 시간 객체를 받게 됩니다. `chrono` 는 `C++ 11` 에 새로 추가된 시간 관련 라이브러리로 기존의 C 의 `time.h` 보다 훨씬 편리한 기능을 제공하고 있습니다. 이에 대해서는 나중 강좌에서 자세히 다루어 보도록 하고, 일단 `100 * index` 밀리초 만큼 쓰레드를 재우기 위해서는
`chrono::milliseconds(100 * index)` 와 같이 전달하면 됩니다.
