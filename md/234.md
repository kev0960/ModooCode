...
**문자** 타입들 간의 연산 (비교, 이동 등등)은 `Traits` 를 통해 정의됩니다. \sidenote{예를 들어 대소문자 구분이 없는 string 을 만들 수 도 있습니다. Traits 에는 문자를 비교하는 함수를 전달해야 하는데, 같은 대소문자 일 때도 true 를 반환하게 만들면 됩니다}
