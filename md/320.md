...
그렇다면 TU 1 을 컴파일 한 목적 코드에는 도대체 어떤 어셈블리가 들어 있을까요?
...
이 부분인데, 원래는 `e8` 뒤에 현재 위치로 부터 얼마만큼 떨어져 있는 곳에 있는 함수를 실행할지 그 오프셋 값이 들어가 있어야 합니다. 하지만 지금은 위와 같이 그냥 0 으로 채워져있죠. 이는 TU 1 의 입장에서 `SomeFunction` 이 어디에 정의되어 있는지 도무지 알 수 없기 때문입니다.

실제 실행 파일에서 함수들이 어디에 배치되는지는 링킹 시에 정해집니다. 따라서 링킹 과정이 이루어지기 전 까지 `e8` 뒤에 어떤 오프셋 값을 넣어야 하는지는 알 수 없습니다. 그래서 위 처럼 그냥 0 으로 채워놓게 됩니다.
...
`readelf` 프로그램은 리눅스에서 `ELF` 파일 정보를 보기 좋게 출력해주는 프로그램입니다. `-r` 옵션을 주게 되면 해당 파일의 **재배치 테이블(Relocation Table)** 정보를 출력하게 됩니다. 이 재배치 테이블 중 `.rela.text` 의 경우 해당 목적 파일에서 링킹 시에 수정해야할 곳의 위치와 어떠한 식으로 수정해야 할 지에 대해서 설명하고 있습니다.
