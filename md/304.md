...
`<chrono>` 에서는 여러가지 종류의 `clock` 들을 지원하고 있습니다. 예를 들어서 일반적으로 현재 컴퓨터 상 시간을 얻어 오기 위해서는 `std::system_clock` 을 사용하면 되고, 좀더 정밀한 시간 계산이 필요한 경우 (예를 들어 프로그램 성능을 측정하고 싶을 때) `std::high_resolution_clock` 을 사용하시면 됩니다.

이들 객체의 이름이 `clock` 이기는 하지만 실제 시계 처럼 *지금 딱 몇 시* 이렇게 이야기 해주는 것이 아닙니다. 그 대신에, 지정된 시점으로 부터 **몇 번의 틱(tick)이 발생 하였는지 알려주는 time_stamp 객체** 를 리턴합니다. 쉽게 말해 `time_stamp` 객체는 `clock` 의 시작점과 현재 시간의 `duration` 을 보관하는 객체라고 생각하시면 됩니다.
...
각 `clock` 마다 정밀도가 다르기 때문에 각 `clock` 에서 얻어지는 `tick` 값 자체는 조금씩 다릅니다. 예를 들어서 `system_clock` 이 1 초에 `1 tick` 이라면, `high_resolution_clock` 의 경우 `0.00000001` 초 마다 `1 tick` 움직일 수 있습니다.
